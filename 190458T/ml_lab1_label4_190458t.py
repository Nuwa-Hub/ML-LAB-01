# -*- coding: utf-8 -*-
"""ml-lab1-label4-190458t.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1A9JlXy7KyQOJP05l3AOVQWF7ZEEFE2tJ
...
accuracy = accuracy_score(test_label4_copy, y_pred_base_test)
precision = precision_score(test_label4_copy, y_pred_base_test, average='weighted', zero_division=1)
recall = recall_score(test_label4_copy, y_pred_base_test, average='weighted')

print(f"Metrics for KNN on test data:")
print(f"Accuracy: {accuracy:.2f}")
print(f"Precision: {precision:.2f}")
print(f"Recall: {recall:.2f}")
print("\n")
...
# Define the number of groups and features per group
num_groups = 5
features_per_group = len(correlation_matrix) // num_groups

# Set the size of the subplots
fig, axs = plt.subplots(num_groups, 1, figsize=(20, 30))  # Adjust figsize for vertical layout
...
for i in range(num_groups):
    ...
    mask_upper = np.triu(np.ones_like(correlation_matrix.iloc[start_idx:end_idx, start_idx:end_idx], dtype=bool))

    # Create triangular heatmap using seaborn
    sns.heatmap(
        correlation_matrix.iloc[start_idx:end_idx, start_idx:end_idx],
        cmap='coolwarm',
        center=0,
        mask=mask_upper,
        annot=True,
...
def create_csv(features, pred_before_fe, pred_after_fe):
  ...
  df.to_csv('/kaggle/working/190458T_label_4.csv', index=False)
